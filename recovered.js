...
async function createDemoRideMatch(rider, payload) {
  const pool = getPool();
  const preferredDriverId = payload.driverId ? String(payload.driverId) : null;
  const pickupLocation = payload.pickupLocation;
  const driverMatch = await findAvailableDriver(preferredDriverId, pickupLocation);
  ...
  const driverId = driverMatch ? driverMatch.profile.user_id : null;
  const status = driverMatch ? "MATCHED" : "SEARCHING";
  const otp = String(Math.floor(1000 + Math.random() * 9000));
  const etaMinutes = driverMatch ? driverMatch.summary.etaMinutes : null;

  await pool.query(
    `INSERT INTO rides (
      id, rider_id, driver_id, pickup, destination, 
      pickup_lat, pickup_lng, destination_lat, destination_lng, 
      co_rider_pickup_distance_meters, estimated_fare, eta_minutes, otp, status
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      rideId, rider.id, driverId, payload.pickup.trim(), payload.destination.trim(),
      ...
      700, estimatedFare, etaMinutes, otp, status
    ]
  );
